				...
				<div id="content" role="main" class="content <?php echo $HTMLkombinat->options['sidebar_blog']; ?>">
					<div class="article-container">
						...
						<header class="entry-header">
							...
							<?php if ( is_day() ) : ?>
								<?php printf( __( 'Daily Archives: %s', 'htmlkombinat' ), '<span>' . get_the_date() . '</span>' ); ?>
							<?php elseif ( is_month() ) : ?>
								<?php printf( __( 'Monthly Archives: %s', 'htmlkombinat' ), '<span>' . get_the_date( _x( 'F Y', 'monthly archives date format', 'htmlkombinat' ) ) . '</span>' ); ?>
								<?php elseif ( is_year() ) : ?>
								<?php printf( __( 'Yearly Archives: %s', 'htmlkombinat' ), '<span>' . get_the_date( _x( 'Y', 'yearly archives date format', 'htmlkombinat' ) ) . '</span>' ); ?>
							<?php else : ?>
								<?php _e( 'Blog Archives', 'htmlkombinat' ); ?>
							<?php endif; ?>
							</p>
						</header>
							<?php /* Start the Loop */ ?>
						...
						<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
							...
							<?php else : ?>
							<article id="no-post" class="post no-results not-found article-container">
								<header class="entry-header">
