				...
				<?php if ( have_posts() ) : ?>
					<?php the_post(); ?>
						<header class="entry-header">
							<p class="sticky-message"><?php printf( __( 'Author Archives: %s', 'htmlkombinat' ), '<span class="vcard"><a class="url fn n" href="' . esc_url( get_author_posts_url( get_the_author_meta( "ID" ) ) ) . '" title="' . esc_attr( get_the_author() ) . '" rel="me">' . get_the_author() . '</a></span>' ); ?></p>
						<?php rewind_posts(); ?>
						<?php if (get_the_author_meta('description') != '') :
							$avatar = get_avatar(get_the_author_meta('user_email'),$HTMLkombinat->avatarSize); ?>
