				...
				<div id="content" role="main" class="content <?php  echo $HTMLkombinat->options['sidebar_blog']; ?>">
					...
					<?php if ( have_posts() ) : ?>
						<header class="entry-header">
							<p class="sticky-message"><?php printf( __( 'Tag Archives: %s', 'htmlkombinat' ), '<span>' . single_cat_title( '', false ) . '</span>' ); ?></p>
						<?php
							$tag_description = tag_description();
							if ( ! empty( $tag_description ) )
								echo apply_filters( 'tag_archive_meta', '<div class="metabar"><p>' . $tag_description . '</p></div>' );
						?>
						</header>
							<?php /* Start the Loop */ ?>
